...
contract('hexgon farm test set allocPoint', function (accounts) {
...
/////////////////////////////////////////////////////////////////////////////
    ...
    it("[0020] stake lp in farm pool,should pass", async () => {
        let poolInfo = await farminst.poolInfo(0);
        console.log("before",poolInfo[0].toString(10), poolInfo[1].toString(10), poolInfo[2].toString(10));
        let accFlakePerShare = poolInfo[0].toString(10);
        let lastRewardTime = poolInfo[1].toString(10);
        let allocPoint = poolInfo[2].toString(10);

        // ////////////////////////staker1///////////////////////////////////////////////////////////
        time.increase(1000);
        res = await farminst.massUpdatePools([0]);
        assert.equal(res.receipt.status, true);

        res = await farminst.setFlakePerSecond(0);
        assert.equal(res.receipt.status, true);


        poolInfo = await farminst.poolInfo(0);
        console.log("after",poolInfo[0].toString(10), poolInfo[1].toString(10), poolInfo[2].toString(10));
        let accFlakePerShare1 = poolInfo[0].toString(10);
        let lastRewardTime1 = poolInfo[1].toString(10);
        let allocPoint1 = poolInfo[2].toString(10);

        assert.equal(accFlakePerShare!==accFlakePerShare1,true,"accFlakePerShare should not be same");
        assert.equal(lastRewardTime!==lastRewardTime1,true,"lastRewardTime should not be same");
