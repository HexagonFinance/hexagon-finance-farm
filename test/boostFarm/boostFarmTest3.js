...
//set farm///////////////////////////////////////////////////////////
        ...
        lpgaugeAddr = await farminst.lpGauges(0);
        farmNewLp = await LpGauge.at(lpgaugeAddr);
//set boost//////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
    ...
    it("[0010] staker1 lp in farm pool,should pass", async()=>{
        ...
        console.log("prebalance=",preBal.toString(10));
        res = await farminst.deposit(0,VAL_1000,staker1,{from:staker1});
        assert.equal(res.receipt.status,true);
        time.increase(1);
    ...
    it("[0020] staker2 lp in farm pool,should pass", async()=>{
        let poolInfo = await farminst.poolInfo(0);
        console.log(poolInfo[0].toString(10),poolInfo[1].toString(10),poolInfo[2].toString(10));

        // ////////////////////////staker1///////////////////////////////////////////////////////////
        res = await lp.approve(farminst.address,VAL_1B,{from:staker2});
        assert.equal(res.receipt.status,true);

        let preBal = await lp.balanceOf(farminst.address);
        console.log("prebalance=",preBal.toString(10));
        res = await farminst.deposit(0,VAL_1000,staker2,{from:staker2});
        assert.equal(res.receipt.status,true);
        time.increase(1);
    ...
    it("[0030] staker1 newlp transfer to staker3,should pass", async()=>{
        let res = await farmNewLp.transfer(staker3,VAL_1000,{from:staker1});
        assert.equal(res.receipt.status,true);
        time.increase(10);
